...
const NavBar = () => {
    ...
    const handleLogOut = async () => {
        try {
            await axiosPublic.post("http://localhost:5000/clearCookie", user, { withCredentials: true }); 
            await logOut();
        } catch (error) {
            console.log(error);
        }
...
/* 
fixed--> Makes the navbar stay fixed at the top of the screen, even when the user scrolls. It does not move with the page.
🔹 z-10: Sets the z-index of the navbar to 10, meaning it will stay above other elements that have a lower z-index (default is z-0).
Helps prevent it from being overlapped by other content.*/